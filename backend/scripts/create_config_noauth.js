...
(async () => {
  try {
    ...
    const put = await request({ hostname: 'localhost', port: 5163, path: `/api/config/${encodeURIComponent(key)}`, method: 'PUT', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(putPayload) } }, putPayload);
