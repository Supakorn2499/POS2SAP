...
(async () => {
  try {
    ...
    const put = await request({ hostname: 'localhost', port: 5163, path: `/api/debug/config/${encodeURIComponent(key)}`, method: 'PUT', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(putPayload) } }, putPayload);
    console.log('PUT RESPONSE:', JSON.stringify(put, null, 2));

    const get = await request({ hostname: 'localhost', port: 5163, path: '/api/config', method: 'GET' });
